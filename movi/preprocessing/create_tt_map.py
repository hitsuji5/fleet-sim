...
def create_reachable_map(engine):
    ...
    d_max = great_circle_distance(lat0, lon0, lat1, lon1) / 2.0 * 1.01
...
def create_tt_map(engine, reachable_map):
    origin_destins_list = []
    for x, y in state_space:
    ...
    a_size = MAX_MOVE * 2 + 1
    tt_map = np.ones((MAP_WIDTH, MAP_HEIGHT, a_size, a_size),
                     dtype=np.float32) * float('inf')
    for (x, y), tt in zip(state_space, tt_list):
        tt_map[x, y] = np.array(tt).reshape((a_size, a_size))
    ...
    tt_map[np.isnan(tt_map)] = float('inf')
    return tt_map
...
if __name__ == '__main__':
    ...
    print("create tt map")
    tt_map = create_tt_map(engine, reachable_map)
    np.save("{}/tt_map".format(args.data_dir), tt_map)
